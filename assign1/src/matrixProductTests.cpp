...
int main (int argc, char *argv[])
{
	...
	if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " <operation> <matrix_size> [block_size]" << std::endl;
        return 1;
    }

    int op = std::atoi(argv[1]);
    int matrix_size = std::atoi(argv[2]);
    int blockSize = (argc == 4) ? std::atoi(argv[3]) : 0; 
    ...
    switch (op) {
        ...
        case 3:
            if (argc != 4) {
                std::cerr << "Block size not specified for block multiplication." << std::endl;
                return 1;
            }
            printf("Block Size: %d\n", blockSize);
            OnMultBlock(matrix_size, matrix_size, blockSize);
            break;
        case 4:
            OnMultLineParallel1(matrix_size, matrix_size);
            break;
        case 5:
            OnMultLineParallel2(matrix_size, matrix_size);
            break;
        default:
